...
export const validateDoctorProfile = (
    ...
    if (
        ...
        res.status(400).json({
            message:
                "Specialty, experience, clinicAddress, consultationFee, workingHours, and availabilityStatus are required"
        });
        return;
    }
    ...
    if (
        ...
        res.status(400).json({
            message:
                "Consultation fee must be a number greater than or equal to 0"
        });
        return;
    }
...
export const validateDoctorProfileUpdate = (
    req: Request,
    res: Response,
    next: NextFunction
): void => {
